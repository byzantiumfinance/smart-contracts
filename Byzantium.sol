...
contract Byzantium {
	...
	string constant public name = "Byzantium";
	uint8 constant public decimals = 18;
	string constant public symbol = "BYZ";
	uint256 constant private SCALAR_VALUE = 2**64;
	uint256 constant private BURN_RATE = 10;
	uint256 constant private INITIAL_SUPPLY = 1e26;
	uint256 constant private MIN_SUPPLY = 10;
	uint256 constant private MIN_STAKE = 1000;
	...
	function burn(uint256 _tokens) external {
		...
		if (info.totalStaked > 0) {
			_burnedAmount /= 2;
			info.scaledPayoutPerToken += _burnedAmount * SCALAR_VALUE / info.totalStaked;
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		if (totalSupply() - _burnedAmount < INITIAL_SUPPLY * MIN_SUPPLY / 100 || isBurningDisabled(_from)) {
			_burnedAmount = 0;
		}
	...
	function _stake(uint256 _amount) internal {
		...
		require(stakedOf(msg.sender) + _amount >= MIN_STAKE);
	...
	function collect() external returns (uint256) {
		...
		info.users[msg.sender].scaledPayout += int256(_dividends * SCALAR_VALUE);
	...
	function distribute(uint256 _tokens) external {
		...
		require(balanceOf(msg.sender) >= _tokens);
		info.users[msg.sender].balance -= _tokens;
		info.scaledPayoutPerToken += _tokens * SCALAR_VALUE / info.totalStaked;
	...
	function dividendsOf(address _user) public view returns (uint256) {
		return uint256(int256(info.scaledPayoutPerToken * info.users[_user].staked) - info.users[_user].scaledPayout) / SCALAR_VALUE;
	}
